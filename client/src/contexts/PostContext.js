...
export class PostContext extends React.Component {
    ...
    createPost = async (title, content, mediaFile, location, categories, taggedAthletes) => {
        try {
    ...
    getSinglePost = async postId => {
        try {
            const post = await service.get(`/post/single/${postId}`)
    ...
    filterOnBoundsSearch = areaSpot => {
            ...
            if (!post.spot) {
               return 
                ...
                if(post.spot.placeId == areaSpot.place_id || post.spot.location.coordinates.lat >= areaSpot.geometry.bounds.Va.i && post.spot.location.coordinates.lat <= areaSpot.geometry.bounds.Va.j && post.spot.location.coordinates.lng >= areaSpot.geometry.bounds.Qa.i && post.spot.location.coordinates.lng <= areaSpot.geometry.bounds.Qa.j){
